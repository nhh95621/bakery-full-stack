...
function DataErrorState({ title, onRetry }: { title: string; onRetry: () => void }) {
  return (
    <div role="alert" className="border border-terracotta/25 bg-terracotta/5 px-6 py-12 text-center">
      <p className="serif-subtitle text-xl text-primary">{title}</p>
      <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Vui lòng kiểm tra kết nối và thử lại để tiếp tục xem không gian Boulangerie của bạn.</p>
      <Button variant="outline" onClick={onRetry} className="mt-5 border-primary/25 text-primary hover:bg-primary hover:text-primary-foreground">Thử lại</Button>
    </div>
  );
}
...
export default function Account() {
  ...
  const { data: userOrders = [], isLoading: ordersLoading, isError: ordersError, refetch: refetchOrders } = trpc.orders.list.useQuery();
  const { data: userFavorites = [], isLoading: favoritesLoading, isError: favoritesError, refetch: refetchFavorites } = trpc.favorites.list.useQuery();
  const { data: userReviews = [], isLoading: reviewsLoading, isError: reviewsError, refetch: refetchReviews } = trpc.reviews.listMine.useQuery();
      ...
      <main className="container pb-16 pt-8 md:pt-12">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <section aria-live="polite">
            {ordersError || favoritesError || reviewsError ? <DataErrorState title="Không thể tải không gian hồ sơ" onRetry={() => { void refetchOrders(); void refetchFavorites(); void refetchReviews(); }} /> : <>
            {activeTab === "orders" && <div><div className="mb-6 flex items-end justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-terracotta">Les commandes</p><h2 className="serif-title mt-2 text-3xl text-primary">Lịch sử đơn hàng</h2></div><ShoppingBag size={22} className="text-gold" /></div>
              {ordersLoading ? <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : userOrders.length === 0 ? <EmptyState icon={Package} title="Chưa có đơn hàng nào" description="Khám phá những chiếc bánh được hoàn thiện theo mùa cho lần ghé thăm đầu tiên." actionLabel="Khám phá sản phẩm" onAction={startShopping} /> : <div className="space-y-4">{userOrders.map((order) => { const status = orderStatuses[order.status] ?? orderStatuses.pending; return <article key={order.id} className="border border-primary/10 bg-card p-5 shadow-sm transition-shadow hover:shadow-md sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-terracotta">Commande #{order.id}</p><h3 className="serif-subtitle mt-1 text-xl text-primary">Đơn hàng Boulangerie</h3><p className="mt-2 text-sm text-muted-foreground">{new Date(order.createdAt).toLocaleDateString("vi-VN", { day: "2-digit", month: "long", year: "numeric" })}</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${status.className}`}>{status.label}</span></div><div className="mt-5 grid gap-3 border-t border-primary/10 pt-4 text-sm sm:grid-cols-2"><p className="text-muted-foreground"><span className="text-foreground">Giao đến:</span> {order.customerAddress}</p><p className="sm:text-right"><span className="font-semibold text-primary">{currency.format(Number(order.total))}₫</span></p></div></article>; })}</div>}
            </div>}
