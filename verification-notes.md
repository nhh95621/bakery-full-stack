...
- Chuẩn hoá cấu trúc lần cuối: loại bỏ `client/` cũ không được tham chiếu, giữ `frontend/` và `backend/` là hai khối mã ứng dụng. Lệnh kiểm kê xác nhận không còn thư mục `client/` hoặc `server/` và không có tham chiếu `client/` trong mã chạy. `pnpm check`, `pnpm test` (**16 files, 35 passed, 1 skipped**) và `pnpm build` đều đạt. Sau restart lúc 04:35 UTC+7, trang chủ phản hồi bình thường; phần log mới chỉ có entrypoint `backend/_core/index.ts`, OAuth, server-ready và không có lỗi/đường dẫn legacy.
